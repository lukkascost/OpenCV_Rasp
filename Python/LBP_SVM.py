...
for metodo in ["PASSO_ROI_PRETO"]:
        ...
        for percent in passos:
                print percent
                obj = rodada(it,nclasses,nAtrib=256,nTreino=13, nAmostras=50)
...
for percent in passos:
        ...
        p2 = [1, 2, 3, 4, 5, 7, 8, 9, 13, 15, 16, 17, 25, 29, 31, 32, 33, 49, 57, 61, 63, 64, 65, 97, 113, 121, 125, 127, 128, 129, 130, 132, 136, 144, 160, 192, 193, 194, 196, 200, 208, 224, 225, 226, 228, 232, 240, 241, 242, 244, 248, 249, 250, 252, 253, 254, 255,256]
        obj.execIteractions(p2)
        lin2+=  "{:02.04f}\t".format(obj.get_avg_acc()[0][-1][0])
        p2 = [2, 33, 128, 193, 244, 248, 249, 250, 254, 256]
        obj.execIteractions(p2)
