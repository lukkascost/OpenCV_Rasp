...
def Normalizar(BancoDeDados,objetos,atributos):
        ...
        for atrib in range(atributos):                                  ##
                for obj in range(objetos):                              ##
                        if (matNormalizar[atrib][1]-matNormalizar[atrib][0])  == 0:
                                BancoDeDados[obj][atrib] = 0.0
                        else:
...
def GLCM_tipo1(img, classe):
        ...
        gray = 256
        coOccurence = getCoOccurrenceMatrix(img, 256)
...
#----------------------------------------------------------------------
def getLineCoOccurence(img,indexLine,grayscale=256):
        """
        """
        result = np.zeros((1,grayscale))
...
def normalizeLine(line, imgshape):
        return line/((imgshape[0]*(imgshape[1]-1)))

def GLCM_tipo3(img, classe, gray = 256):
        glcm_atributes = np.zeros(26,dtype=np.float64)  
        for i in range(gray):       
                coOccurenceNormalized = normalizeLine(getLineCoOccurence(img,i,grayscale=gray), img.shape)
