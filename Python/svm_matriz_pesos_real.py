...
for stringCaminho in ["PASSO_DECIMACAO","PASSO_ROI","PASSO_ROI_PRETO"]:
        ...
        for percent in range(70,100):
                ...
                addressSave = "Resultados_real/SVM_Vectors"+str(percent)+".txt"
                ########################################################################################################################################################
                ...
                conf_mat_preco = np.zeros((iteracoes,7,7))  ## Tabela contendo a matriz confusao de lucros [x][y][z], iteração[x], elemento da classe[y], lucro[z]
                ...
                for x in range(iteracoes):
                        ...
                        conf_mat_preco[x] = np.multiply(conf_mat[x],pesosPreco)
                        ########################################################################################################################################################
                ...
                Salvar_texto(Resultado,"Resultados_real/"+stringCaminho+"/Result_"+str(percent)+"percent.txt")
                np.save("Resultados_real/"+stringCaminho+"/Acc"+str(percent)+"percentMat",acc_tab_res)
                np.save("Resultados_real/"+stringCaminho+"/EscErr"+str(percent)+"percentMat",esc_err_tab_res)
                np.save("Resultados_real/"+stringCaminho+"/EscAce"+str(percent)+"percentMat",esc_cor_tab_res)
                np.save("Resultados_real/"+stringCaminho+"/ConfMat"+str(percent)+"percentMat",conf_mat)
                np.save("Resultados_real/"+stringCaminho+"/ConfMatP"+str(percent)+"percentMat",conf_mat_preco)
                ########################################################################################################################################################
                ########################################################################################################################################################
